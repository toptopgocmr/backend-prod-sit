...
@section('breadcrumb')
    ...
    <a href="{{ route('equipment.show', $equipment) }}" class="hover:text-primary-500 transition-colors">{{ $equipment->name }}</a>
    <i data-lucide="chevron-right" style="width:12px;height:12px"></i>
...
<style>
    ...
    .type-option{flex:1;border:1.5px solid #e5e7eb;border-radius:12px;padding:12px;cursor:pointer;transition:border-color .15s,background .15s;background:#f9fafb;display:flex;align-items:center;gap:10px;position:relative}
    ...
    .status-option{flex:1;border:1.5px solid #e5e7eb;border-radius:12px;padding:10px 14px;cursor:pointer;transition:border-color .15s,background .15s;background:#f9fafb;display:flex;align-items:center;gap:8px;position:relative}
    .status-option input{position:absolute;opacity:0}
    .status-option:has(input[value="operationnel"]:checked){border-color:#16a34a;background:#f0fdf4}
    .status-option:has(input[value="en_panne"]:checked){border-color:#dc2626;background:#fef2f2}
    .status-option:has(input[value="en_maintenance"]:checked){border-color:#d97706;background:#fffbeb}
</style>
@endpush
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6 fade-in">
        <div>
            <h2 class="text-xl font-bold text-dark font-display">Modifier l'équipement</h2>
            <p class="text-sm text-gray-400 mt-0.5">{{ $equipment->name }}</p>
        </div>
        <a href="{{ route('equipment.show', $equipment) }}"
           class="inline-flex items-center gap-2 text-sm text-gray-500 border border-gray-200 px-3 py-2 rounded-xl transition-colors">
    ...
    <form method="POST" action="{{ route('equipment.update', $equipment) }}">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="brand" value="{{ old('brand',$equipment->brand) }}"
                           class="form-input" placeholder="Marque…">
                </div>
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="location" value="{{ old('location',$equipment->location) }}"
                               class="form-input" style="padding-left:34px" placeholder="Atelier, bureau…">
                    </div>
                </div>
            </div>
        </div>
        ...
        {{-- Statut --}}
        <div class="section-card fade-in">
            <p class="section-title"><i data-lucide="activity" style="width:14px;height:14px"></i>Statut actuel</p>
            <div class="flex gap-2">
                @foreach(['operationnel'=>['Opérationnel','check-circle','text-green-600'],'en_panne'=>['En panne','alert-triangle','text-red-600'],'en_maintenance'=>['En maintenance','tool','text-amber-600']] as $val=>[$lbl,$icon,$tc])
                <label class="status-option">
                    <input type="radio" name="status" value="{{ $val }}" @checked(old('status',$equipment->status)===$val)>
                    <i data-lucide="{{ $icon }}" style="width:15px;height:15px" class="{{ $tc }}"></i>
        ...
        {{-- Type --}}
        <div class="section-card fade-in">
            <p class="section-title"><i data-lucide="layers" style="width:14px;height:14px"></i>Type</p>
            <div class="grid grid-cols-2 gap-2 sm:grid-cols-3">
                ...
                <label class="type-option">
                    <input type="radio" name="type" value="{{ $val }}" @checked(old('type',$equipment->type)===$val)>
                    <span class="w-7 h-7 rounded-lg {{ $bg }} flex items-center justify-center shrink-0">
                        <i data-lucide="{{ $icon }}" style="width:14px;height:14px" class="{{ $tc }}"></i>
                    </span>
                    <span class="text-xs font-semibold text-dark">{{ $lbl }}</span>
                </label>
                @endforeach
        ...
        {{-- Maintenance & Notes --}}
        <div class="section-card fade-in">
            <p class="section-title"><i data-lucide="clock" style="width:14px;height:14px"></i>Maintenance</p>
            <div class="mb-4">
                <label class="form-label">Intervalle de maintenance (jours) <span class="text-red-400">*</span></label>
                <input type="number" name="maintenance_interval_days"
                       value="{{ old('maintenance_interval_days',$equipment->maintenance_interval_days) }}"
                       min="1" required class="form-input w-32">
            </div>
            <div>
                <label class="form-label">Notes</label>
                <textarea name="notes" rows="3" class="form-input" style="resize:none"
                          placeholder="Informations complémentaires…">{{ old('notes',$equipment->notes) }}</textarea>
            </div>
        </div>
