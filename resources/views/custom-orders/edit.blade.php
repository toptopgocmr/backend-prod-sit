    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden"
                 x-data="{ mode: '{{ $customOrder->measurement_id ? 'existing' : 'new' }}', newLabel: '' }">
                ...
                <div class="p-5 space-y-4">
                    ...
                    <div x-show="clientId && mode==='existing'" x-transition>
                        ...
                        <div x-show="clientMeasurements.length === 0"
                            ...
                            Aucune fiche pour ce client. Créez-en une ci-dessus.
                        </div>
                    </div>
                    ...
                    <div x-show="clientId && mode==='new'" x-transition class="space-y-4">
                        <div>
                            ...
                            <input type="text" name="new_measurement_label" x-model="newLabel"
                                   placeholder="Ex : Mesures juin 2026, Robe mariage..."
                                   class="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm text-dark
                                          focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all">
                        </div>
                        ...
                        <div x-show="gender === 'femme'" class="space-y-4">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                @foreach([
                                    ...
                                    'f_longueur_veste'=>'Longueur veste','f_tour_encolure'=>'Tour encolure','f_hauteur_fessier'=>'Hauteur fessier',
                                ] as $field => $label)
                                <div>
                                    <label class="block text-xs text-gray-500 mb-1">{{ $label }} <span class="text-gray-300">cm</span></label>
                                    <input type="number" name="new_measurement[{{ $field }}]" min="1" max="999" step="0.5" placeholder="—"
                                           class="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm text-dark text-right
                                                  focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all">
                                </div>
                                @endforeach
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                @foreach([
                                    'f_tour_ceinture'=>'Tour de ceinture','f_tour_bassin'=>'Tour de bassin',
                                    'f_hauteur_bassin'=>'Hauteur bassin','f_longueur_assise'=>"Longueur d'assise",
                                    'f_fourche_devant'=>'Fourche devant','f_fourche_dos'=>'Fourche dos',
                                    'f_entrejambe'=>'Entrejambe','f_longueur_pantalon'=>'Longueur pantalon',
                                    'f_tour_cuisse'=>'Tour de cuisse','f_largeur_bas'=>'Largeur du bas',
                                ] as $field => $label)
                                <div>
                                    <label class="block text-xs text-gray-500 mb-1">{{ $label }} <span class="text-gray-300">cm</span></label>
                                    <input type="number" name="new_measurement[{{ $field }}]" min="1" max="999" step="0.5" placeholder="—"
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                @foreach([
                                    ...
                                    'f_jupe_longue'=>'Jupe longue','f_jupe_genoux'=>'Jupe genoux','f_jupe_trois_quarts'=>'Jupe trois quarts',
                                ] as $field => $label)
                                <div>
                                    <label class="block text-xs text-gray-500 mb-1">{{ $label }} <span class="text-gray-300">cm</span></label>
                                    <input type="number" name="new_measurement[{{ $field }}]" min="1" max="999" step="0.5" placeholder="—"
                        ...
                        <div x-show="gender === 'homme'" class="space-y-4">
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                @foreach([
                                    'h_epaule'=>'Épaule','h_manche_longue'=>'Manche longue','h_manche_courte'=>'Manche courte',
                                    'h_tour_poitrine'=>'Tour de poitrine','h_tour_taille_ventre'=>'Tour taille/ventre',
                                    'h_carrure_dos'=>'Carrure dos','h_longueur_chemise'=>'Longueur chemise',
                                    'h_longueur_veste'=>'Longueur veste','h_contour_manche'=>'Contour manche',
                                    'h_tour_col'=>'Tour de col','h_longueur_devant'=>'Longueur devant',
                                ] as $field => $label)
                                <div>
                                    <label class="block text-xs text-gray-500 mb-1">{{ $label }} <span class="text-gray-300">cm</span></label>
                                    <input type="number" name="new_measurement[{{ $field }}]" min="1" max="999" step="0.5" placeholder="—"
                        ...
                        <div class="flex items-center gap-2 px-3 py-2 bg-purple-50 rounded-xl text-xs text-purple-700">
                            <input type="checkbox" name="new_measurement_default" id="meas_default_edit" value="1" class="rounded">
                            <label for="meas_default_edit" class="font-semibold cursor-pointer">Définir comme fiche par défaut pour ce client</label>
                        </div>
