...
@section('breadcrumb')
    ...
    <span class="text-gray-600 font-medium">Nouvelle dépense</span>
@endsection
...
<div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-display font-bold text-dark">Nouvelle dépense</h2>
                <p class="text-xs text-gray-400">Enregistrer une sortie de caisse</p>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="payment_method" required
                            class="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition-all bg-white @error('payment_method') border-red-400 @enderror">
                ...
                <div>
                    ...
                    <input type="date" name="expense_date" value="{{ old('expense_date', date('Y-m-d')) }}" required
                           class="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition-all @error('expense_date') border-red-400 @enderror">
                    @error('expense_date')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" rows="3" placeholder="Détails supplémentaires..."
                              class="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition-all resize-none">{{ old('notes') }}</textarea>
                </div>

                {{-- Justificatif --}}
                <div class="md:col-span-2">
                    <label class="block text-sm font-semibold text-dark mb-1.5">Justificatif (photo reçu)</label>
                    <input type="file" name="receipt_photo" accept="image/*"
                           class="w-full px-3.5 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all file:mr-3 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20">
